// src/app/dashboard/[businessName]/appointments/_components/CustomCalendar.tsx
'use client';
...
import { type AppointmentForCalendar } from '@/(features)/appointments/appointment.queries'; // Import the new type

// --- Type Definitions ---
// The local Appointment type has been REMOVED.

type CustomCalendarProps = {
  appointmentsData: AppointmentForCalendar[];
  businessName: string;
};

// --- Sub-Components ---
function AppointmentEvent({ app }: { app: AppointmentForCalendar }) {
  // Use the new type here as well
  ...
  // You can now display more info, for example:
  const description =
    app.items?.[0]?.description ||
    app.items?.[0]?.service?.name ||
    `${app.vehicle?.year} ${app.vehicle?.make} ${app.vehicle?.model}`.trim() ||
    'Service';

  return (
    <Link
      href={`/dashboard/appointments/${app.id}/edit`} // This route doesn't exist yet, but we'll build it out later.
...
// The main CustomCalendar component function remains unchanged, as it just receives the typed prop.
export default function CustomCalendar({
  ...
  const handleEndSelection = () => {
    if (
      ...
      // This will need to be updated when we build the "new appointment" form.
      // For now, it correctly navigates.
      const endTime = addMinutes(selection.end, 30).toISOString();
      router.push(
        `/dashboard/${businessName}/appointments/new?start=${startTime}&end=${endTime}`
      );
    }
  ...
  // The rest of the component's JSX and logic remains the same...
  // ...
  ...
  return (
    <div className='flex flex-col h-full bg-card rounded-lg'>
      ...
      <div className='flex-1 relative'>
        <div
          className='absolute inset-0 overflow-auto no-scrollbar rounded-bl-lg'
